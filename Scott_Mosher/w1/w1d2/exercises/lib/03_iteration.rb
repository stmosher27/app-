...
def substrings(string)
  ...
  for i in 0...string.length
		ansArr << string[i] unless ansArr.include?(string[i])
		for u in i+1...string.length
			ansArr << string[i..u] unless ansArr.include?(string[i..u])
		end
	end
  ansArr
end
...
def subwords(word, dictionary)
  ...
  arr.each do |w|
    if dictionary.include?(w)
      answerArr << w unless answerArr.include?(w)
    end
  end
  answerArr
end
...
class Array
  ...
  def my_inject(&blk)
    num = self.first
    self.drop(1).my_each { |el| num = blk.call(num, el) }
    num
  end
end
...
def concatenate(strings)
  strings.inject("") {|ans, word| ans + word }
end
